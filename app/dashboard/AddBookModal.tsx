...
/* ── Inline spinner (no external component needed) ── */
function Spinner() {
  return (
    <span style={{
...
export default function AddBookModal() {
  ...
  const resetForm = () => {
    ...
    setCategory(''); setStock(1); setRak(''); setJpgUrl('');
  };
  ...
  const handleAddBook = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('books').insert([
      { title, author, publisher, category, stock, rak, jpg_url: jpgUrl }
    ]);
    setLoading(false);
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-left my-8 relative">
            ...
            <form onSubmit={handleAddBook} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Link E-Book (JPG)</label>
                  <input type="url" value={jpgUrl} onChange={(e) => setJpgUrl(e.target.value)}
                    className="w-full bg-white border border-slate-200 px-4 py-3 rounded-xl focus:ring-2 focus:ring-[#1B4332] focus:border-[#1B4332] outline-none transition-all font-medium text-slate-800"
                    placeholder="https://..." />
                </div>
              </div>
