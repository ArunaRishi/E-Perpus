...
export default function QRCodeModal({ book, isLoggedIn = false }: { book: any, isLoggedIn?: boolean }) {
  ...
  // 1. Logika URL: Gunakan IP laptop agar bisa diakses HP di wifi yang sama
  // Jika sudah online (production), gunakan domain resmi
  const appUrl = typeof window !== 'undefined' && window.location.hostname === 'localhost'
    ? "http://192.168.1.29:3000" 
    : "https://e-perpus.kejaksaan.go.id";

  // 2. Pastikan ID buku ada, jika tidak, QR tidak akan valid
  const qrData = book?.id ? `${appUrl}/buku/${book.id}` : `${appUrl}/404`;
          ...
          >
            ...
            <div className="flex-1 p-6 md:p-10 flex flex-col justify-between">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                {isLoggedIn ? (
                  <Link href="/dashboard?tab=buku" className="flex-1 text-center py-4 bg-[#1B4332] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#123023] shadow-md flex justify-center items-center gap-2">
                    <span className="text-sm">📚</span> Pinjam Fisik
                  </Link>
                ) : (
                  <Link href="/login" className="flex-1 text-center py-4 bg-slate-800 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black shadow-md flex justify-center items-center gap-2">
            ...
            <div className="w-full md:w-72 bg-slate-50 p-6 md:p-10 flex flex-col items-center justify-center border-l border-slate-100">
              ...
              <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 mb-6 w-40 h-40 flex items-center justify-center">
                {/* 3. Validasi: Jika book.id tidak ada, tampilkan pesan error, bukan QR rusak */}
                {book?.id ? (
                  <img
                  ...
                  <div className="text-[8px] text-center text-red-400 font-bold">DATA BUKU TIDAK VALID</div>
                )}
              </div>
              ...
              <p className="text-[9px] text-slate-400 font-mono tracking-widest uppercase truncate w-full text-center bg-white py-2 rounded-lg border border-slate-100 mb-2">
               ID: {book?.id ? String(book.id).substring(0, 8) + '...' : "XXXXXXXX"}
              </p>
