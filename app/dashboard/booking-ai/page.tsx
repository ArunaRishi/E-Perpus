...
export default function BookingAdminPage() {
  ...
  const filtered = enrichedLoans.filter((l) => {
    ...
    const matchTab =
      tab === 'semua'       ? true :
      tab === 'ai_only'     ? l.borrowed_via === 'AI_JaMiLa' :
  ...
  const stats = {
    ...
    viaAI:       enrichedLoans.filter(l => l.borrowed_via === 'AI_JaMiLa').length,
  };
  ...
  const TABS: { key: Tab; label: string; count?: number }[] = [
    ...
    { key: 'ai_only',     label: '🤖 Via JaMiLa',  count: stats.viaAI },
  ];
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <p className="text-emerald-400 text-[10px] font-black uppercase tracking-[0.25em] mb-1">
                E-Perpustakaan · Kejaksaan Negeri Takalar
              </p>
              <h1 className="text-3xl font-black text-white leading-tight">
                Manajemen Sirkulasi
              </h1>
              <p className="text-emerald-200/70 text-sm font-medium mt-1">
                Kelola peminjaman buku — termasuk yang masuk lewat JaMiLa AI
              </p>
            </div>
            <button
              onClick={fetchLoans}
